...
import '../../core/constants/unicode_emoji_data.dart';
...
class _EmojiPickerSheetState extends ConsumerState<EmojiPickerSheet>
  ...
  Widget _buildUnicodeTab(ScrollController scrollController) {
    if (_query.isNotEmpty) {
      final all = unicodeEmojiCategories.values.expand((l) => l).toList();
      return _unicodeEmojiGrid(all, scrollController);
    }
    final cats = unicodeEmojiCategories.keys.toList();
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              children: cats.map((c) {
                final list = unicodeEmojiCategories[c]!;
